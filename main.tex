...
First, we start by clustering the sequences based on pairwise distances, for example using the UCLUST algorithm \cite{Edgar2010-nv}.
...
\begin{algorithm}
    ...
    \end{algorithmic}
    \begin{algorithmic}
    \Function{doBatchStep}{$R, m, \mathbf c$}
    \For{$i = 1, \dots, m$}:
...
We see that while the nearest neighbor approximation routine greatly reduces the runtime, there is a consistently higher resultant KL divergence to the true distribution.
This is most likely due to the tail bias discussed above.
There is some evidence the KL divergence as well as its variance decrease as the tolerance decreases, but the degree of significance is unclear.

Next we investigate the effect of dataset size on the performance of Algorithm \ref{DistributionAveraging}.
For sample sizes $s \in \{\exp(5), \dots, \exp(9)\}$, we subsample \texttt{p\_f1} without replacement to $s$ sequences and compute the pairwise distance distribution of CDR3 sequences for the full subsampled dataset as well as those given by tolerances $\varepsilon \in \{0.1, 0.01, 0.001\}$.  
We perform this experiment 10 times for each $s$.
Boxplots of the KL-divergence by log(size) and tolerance over all trials are displayed in Figure \ref{fig:Sizes}.
\begin{figure}
    ...
    \label{fig:Sizes}
\end{figure}
The figure shows a possible downward trend in the effect of dataset size on the KL-divergence for each choice of tolerance, although it is difficult to tell exactly what the limiting behavior is.
This suggests that the accuracy of our procedure may increase as the dataset size increase, which may be the result of a more accurate and robust clustering step in \ref{NNDistributionAveraging}.
