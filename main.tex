...
%EM Do we actually address the following complexities with sumrep?
%EM I'd suggest replacing the rest of this paragraph with something like "without further processing, we just have a big list of DNA sequences." Even after annotation we then have gene names and CDR3 sequences, which still aren't immediately obvious how to compare.
...
%EM I'd suggest that talking about k-mers is getting us off track here. I understand that you want to say that there are other ways of going about comparing things (which have their disadvantages), but it seems like a large deviation for the introduction. Perhaps better in Discussion?
%EM If you agree, then we can say here that a good and popular approach is to summarize various aspects of the data in terms of numerical summaries which can then be easily compared. For example...
...
%EM I rewrote some of the following in a less formal way. I hope you don't mind.
In addition to comparing sequencing data sets, summary statistics can also be used to compare sequencing data sets to probabilistic models to which they have been fit.
...
%EM Here we need a paragraph saying that there are no unified packages dedicated to the task of calculating and comparing summary statistics for rep-seq data.
%EM Perhaps point out that immcantation does a lot of these things, but there aren't any comparison functions and you can't ask alakazam to just go out and calculate everything then come back with plots describing differences between repertoires.
...
%EM I'd like to put the information here that most users will want to know, which is currently at the top of the Results section.
%EM I looked for other software papers in Frontiers and found that https://paperpile.com/shared/qnTKS9 doesn't have a Methods section at all! https://www.frontiersin.org/about/author-guidelines seems to say that headings are just guidelines.
%EM I'd like to propose moving most of this to a Results section, and put the Methods last and use it to describe things that immunologists won't care about.
...
%EM I don't know if the rest of this paragraph adds much?
...
%EM I know that a lot of email ink was spilled on this paragraph, but given that the reader doesn't have that context does it make sense to include given that we have no summaries in this category?
...
%EM If we want to use the "level x" terminology then we should add it to the table. If not, then let's just say "The first group of statistics..."
...
\texttt{sumrep} makes it easy to compare each summary statistic between two repertoires by employing some metric or divergence appropriate for the summary in question.
%EM The ability to make comparison is a great thing for users, so perhaps we should expand a little? Like say that for every function like CDR3Length there is a CDR3LengthCompare or whatnot?
%EM We definitely want to say that people can just do "compareRepertoires" and lots of good things happen.
We elect the Jenson-Shannon (JS) divergence for most of our comparison functions.
%EM the formulas can get buried in a Methods section.
%EM On the other hand we really need to explain how each repertoire gives a _distribution_ of values of summary statistics, and that sumrep can compare these distributions of values. A cartoon is probably needed.
%EM Then you can say that JS is a standard and useful means of comparing distributions on numbers, and l1 is a good way of comparing distributions on categories.
%EM We can consider adding an intuitive explanation of KL.
...
%EM I'd suggest keeping an immmunologist-friendly version of this in the Results, and kick the more technical parts to the Methods. We certainly want prospective users to appreciate that we are subsampling, and that for most summaries this means just injecting a little noise into the distribution we are comparing.
...
%EM If we rearrange methods and results as I propose, then we'll have some results from between-repertoire comparison first, then I can add this "also" below.
...
%EM As described in the introduction, we are motivated to do this because...
...
%EM We could throw some of these details in the methods, with a one-sentence summary here.
...
\begin{figure}
%EM Wow, the binwidth isn't looking so great for a lot of these. I'm sure that you have worked on this without luck. If it's not easy, perhaps we just put in the CDFs? In any case only one of them should go in the main text.
...
%EM Is this even worth including then? It sounds just like an artifact.
...
%EM I guess I'd prefer to have the real-data analysis happen first.
...
%EM This ref is giving ??. Please check that latex is compiling cleanly.
...
\begin{figure}
    ...
    \begin{subfigure}[H]{\textwidth}
    	\vspace{1em}
		\input{summary.tex}
		\caption{Numerical summaries of one trial run for each approximate nearest neighbor distance distribution, and the true pairwise distance distribution.}
...
%BJO - Please ignore the following paragraph for now

%Next, we compute the nearest neighbor distribution of full query sequence reads, using the same settings as before.
%
