...
\title{Immune receptor repertoire comparison and model validation}
...
%EM Perhaps you can describe "sum of absolute differences" as a metric on its own and then make your way to VDJ? That would be more parallel with the above.
...
\end{equation}
In words, \eqref{SAD} iterates over each element $s$ in some set $\mathcal S$, calculates the count $c$ of $s$ within repertoires $R_1$ and $R_2$ respectively, takes the absolute difference of counts, and appends this to a rolling sum. 
...
%EM This would be a good place to put your "efficient calculation by subsampling" approaches.
...
%EM for folks just reading because they are interested in the package, you could start by just describing
%BJO Sorry, not sure what you mean with the above.
...
Our strategy is anaologous to the one above but with the inclusion of a separate path for \texttt{igblast} annotations.
