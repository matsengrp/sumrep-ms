...
This is reasonable since each batch iteration of \ref{alg:NNDistributionAveraging} must compute the nearest neighbor distance from each sequence in batch $B$ to the full repertoire $R$, which certainly increases in time complexity as $R$ increases.
...
Figure~\ref{fig:IgorWorkflow} illustrates the \texttt{igor} model validation workflow, which is similar to \ref{fig:PartisWorkflow}.
