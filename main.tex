...
For example, several studies reduce a set of nucleotide sequences to $k$mer distributions for classification of immunization status or disease exposure \cite{Madi2014-lt, Ostmeyer2017-xg, Heather2017pf}, where a $k$mer is a nucleotide sequence motif of size $k$.
These $k$mer distributions can then be compared via sequence-based distances, but still comprise a large space and lose important positional information along the sequence.
...
While many biologically interpretable summaries such as biophysiochmical properties exist and have been widely applied \cite{Chowell2015-gj, Ostmeyer2019-vf, Wu2011-es, Wu2010-dc}, these are often examined at the sequence level rather than the repertoire level.
...
%BJO - Tried to use as little jargon as possible while accurately describing some of these statistics. Please hack away as necessary.
\texttt{sumrep} contains many types of summaries, including nucleotide sequence-level summaries (pairwise distances, hotspot motif counts, etc.), rearrangement summaries like insertion and deletion lengths, and many biophysiochemical properties applicable to the amino acid sequences of particular receptor regions.
The Atchley factors are a set of five functions of an amino acid sequence derived using a statistical technique called factor analysis \cite{Atchley05-hj}.
The Kidera factors are a similarly-constructed set of ten functions of amino acid sequences, which were derived using dimension reduction techniques of a large multivariate analysis of amino acid properties \cite{Kidera85-ss}.
...
Author Jason A. Vander Heiden was employed by company Genentech Inc.
