...
The fourth group requires a inferred phylogeny for each clonal family of an Ig data set.
...
\begin{figure}
    ...
    \end{subfigure}
    \;
    %EM It'd seem to me to make more sense to use https://github.com/matsen/talks/blob/gh-pages/figures/sumrep/paired-summary-stats.svg here. What do you think?
    \begin{subfigure}[c]{0.48\linewidth}
    ...
    \end{subfigure}
    \caption{Cartoon of our summary statistic and divergence framework, and how this can be applied to validation of repertoire simulators. Steps (a) and (b) can be applied to compare arbitrary data sets, while (c) and (d) show how sumrep can be used for model comparison.}
...
%EM When you say "new set" it sounds like this would be a different set of data, but it seems like it's the same Gupta data, no? Please clarify.
...
%EM Did you actually restrict to productive sequences for the fitting? If so, fix the below.
Thus we fit the \igor\ model with all sequences (which are dominated by productive sequences) and during simulation restricted to productive sequences.
...
%EM Definitely. Provide a high-level overview of what the LRADs are here so folks don't have to refer to methods.
%EM I'd suggest pulling the equation refs out of the caption into the text here.
