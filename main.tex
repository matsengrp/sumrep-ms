...
While many biologically interpretable summaries such as biophysiochmical properties exist and have been widely applied \cite{Chowell2015-gj, Ostmeyer2019-vf, Wu2011-es, Wu2010-dcc}, these are often examined at the sequence level rather than the repertoiree level.
...
\end{tabular}
    ...
    \caption{Currently supported summary statistics grouped by their respective degrees of assumed post-processing. Annotation denotes whether annotation of the V(D)J germline segment is required, Clustering denotes whether clonal clustering is required, and Phylogeny denotes whether lineage tree inference is required. ``Tool-provided'' means that the summary can be directly computed from the output of an annotation tool; for example, the CDR3 length distribution is exactly the frequencies of values in the \texttt{junction} column of the annotated dataset.}
