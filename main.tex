...
%EM seems strange to say that good things come from lasso when we penalize large coefficients in the following sentence. I'd say the goodness concerns the small coefficients.
%BJO How does this sound? vvv
While it's difficult to say exactly the level of correlation of each summary by the lasso result alone, since the lasso is a regularized version of least-squares, our intuition is that the nice properties of least-squares combined with the lasso's ability to diminish less relevant coefficients leads to a subset of covariates that are generally informative.
