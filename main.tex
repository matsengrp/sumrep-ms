...
We show that deletion and insertion lengths resulting from VDJ recombination tend to be more discriminative characterizations of a repertoire than summaries characterizing amino acid composition of the junction region.
...
We investigate the effectiveness of various summary statistics in distinguishing between different observed repertoires as well as between simulated and observed data, and show that many summaries are able to differentiate between subject as well as time after vaccination within a subject.
...
The general framework of comparing summary statistics between repertoires $R_1$ and $R_2$ is illustrated in Figures~\ref{fig:SummaryCartoon} and \ref{fig:DivergenceCartoon}.
A given summary $s$ is applied separately to $R_1$ and $R_2$, which for most summaries yields a distribution of values.
These two resultant distributions can be compared using a divergence $\mathcal D$ that is tailored to the nature of $s$.
...
The result is an approximation to the full distribution, which trades some noise for very substantial efficiency gains for large datasets.
...
\begin{figure}
    ...
    $
    \caption{
    We can compare these distributions using a statistical divergence $\mathcal D$, which takes two distributions and outputs a nonnegative scalar.
    }
...
To examine the ability of various summary statistic to distinguish between differences in TCR repertoires by covariate, we performed a multidimensional scaling (MDS) analysis of summary divergences using data from \cite{Pogorelyy2018-ak}.
...
\begin{figure}
    ...
    %BJO Well, another name for classical MDS (which is done for both of these analyses) is principal coordinates analysis, so these labels can still apply. But, I've changed the labels in both plots to be less ambiguous.
