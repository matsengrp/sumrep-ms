...
\affil[5]{Center of Life Sciences, Skolkovo Institute of Science and Technology, Moscow, Russia}
\affil[6]{Department of Bioinformatics and Computational Biology, Genentech, Inc., South San Francisco, CA}
\affil[7]{Genomics of Adaptive Immunity Department, Shemyakin and Ovchinnikov Institute of Bioorganic Chemistry, Moscow, Russia}
\affil[8]{Department of Molecular Technologies, Pirogov Russian National Research Medical University, Moscow, Russia}
...
We show that deletion and insertion lengths resulting from V(D)J recombination tend to be more discriminative characterizations of a repertoire than summaries that describe the amino acid composition of the junction region.
...
This can be a highly involved task, and so it is common to simply compare the gene usage frequencies and CDR3 length distributions of repertoire \cite{Miqueu2007-lk,Larimore2012-lo}, leaving the full richness of CDR3 sequence and potentially interesting aspects of the germline-encoded regions unanalyzed.
...
We wish to facilitate the use of biologically interpretable summary statistics to capture many different aspects of AIRR-seq repertoires.
...
\caption{Currently supported summary statistics grouped by their respective degrees of assumed post-processing. Annotation denotes whether annotation of the V(D)J germline segment is required, Clustering denotes whether clonal clustering is required, and Phylogenic denotes whether lineage tree inference is required.}
...
%WDL 'pairwise aligned' does not feel right to me. I think it should be IMGT-aligned. or aligned to the IMGT numbering system (in other words, what the other half of the pair is)
%CAS A standard alignment like IMGT would probably make sure that all the statstics are exactly as intended, but the AIRR schema only requires alignment to whatever is in the `germline_alignment` field. Anyway, I agree with William that this probably needs a bit more clarification.
...
%CAS probably not a big deal, but in the rendered PDF you sent around, `plotCDR3LengthDistribution` is sticking out into the margin for some reason.
...
\subsection*{Application of summary statistics to real data}
%CAS I've reorganized this a bit, because it felt a bit sudden/unexplained. In trying to state more clearly why this is hear, it also felt natural to merge the T and B analyses into one section, but I don't feel strongly about that part.
...
Next, \texttt{sumrep} is used compute the summary statistics listed in Table~\ref{tab:SummaryStatistics} for each dataset and compare these summaries between each pair of datasets (Figure~\ref{fig:DataSimSummary}).
...
%CAS I can't help but notice that there seems to be a bit of correlation between LRAD-data and the lasso rankings. Based on Appendix D, I guess it might just be a coincidence, but perhaps something to add to the discussion?
...
The Kidera factors of the junction region were also replicated well, despite CDR3 length being one of the least accurately replicated statistics.
Scores for other junction-based statistics besides Kidera factors ranged from mildly good to mildly bad, with the GRAVY index distribution being the best junction-based statistic (excluding Kidera factors) and charge distribution being the worst.
...
%CAS Isn't this kind of a big deal? If I understand correctly, it seems to suggest that IGoR's model is missing some dimension(s) along which real repertoires vary, causing its simulation to look generally more "average" than real life. I suppose it's probably a small effect, but still suggestive of avenues for future research, at least...
...
The other statistics yielded scores ranging from slightly to very negative, with many mutation-related summaries like positional distance between mutations and hot and cold spot counts being poorly-captured.

%WDL poor capture of positional distance between mutations and pairwise distance may be related to the decision to select a single representative from each clonal family (or rather, the inability of partis to model clonal abundance, if I have got that right. I think this is an important limitation that is worth drawing out.
...
As described in Results, we trained on productive sequences and restricted the data to productive sequences for simulation.
...
Before running summary comparisons, we downsample to one receptor per clonal family to get a dataset consisting of unique clonotypes, for both the observed and simulated datasets.
...
%WDL If I've understood this correctly, we're not attempting to compare the size distribution of clonal families in the simulated and observed datasets, because we know they won't correspond. That feels like an important limitation to me and I think we should mention it in the Results section. The justification here seems a little weak (what do we mean by 'true repertoire dynamics'?) but in any case I don't see why we would try to justify it: sumrep's job is to do the comparison.
...
%WDL for consistency, suggest you list here the studies partis was applied to
...
The \texttt{tcR}~\cite{Nazarov2015-ok} and \texttt{bcRep}~\cite{Bischof2016-fn} packages for R includes methods for retrieving and comparing gene usage summaries, computing clonotype diversity indices, and visualizing various repertoire summaries.
...
The \igor~\cite{Marcou2018-du} software features an algorithm for summarizing statistics of the V(D)J rearrangement process, however, the its main focus is on learning the basic model for non-productive T- and B-cell repertoire and it does not provide any built-in methods for comparing inferred models between datasets.
%BJO - Erick, this was your todo, FYI. I could try but I'm sure you have a better idea of where to look than I do.
%EM Let's open the question up to the group.
%CAS - done TODO: Look for others.
...
The tool can be also used to detect systemic biases between different library preparation protocols and control for batch effects that can confound meta-analysis of AIRR-seq data.
...
%MS - some of the factors are already computed for CDR3 only, so better say extending the analysis to regions other than CDR3
%MS - maybe we should say that we plan to integrate with AIRR format and provide an easy-to-use R markdown template that only requires AIRR-format dataset with certain mandatory fields, so that sumrep can be in future used as a downstream analysis method for any AIRR-compliant software/workflow
...
%CAS Are the sample sizes in powers of `e` or powers of 10? To me, exp(*) implies the former...
...
%CAS Yes, but what are the defaults? Do hot/cold spot distributions get the approximation unless the user says otherwise? Same question about Algorithm 2, below.
...
%CAS Same question about powers of e vs 10...
...
%CAS - I suggest striking the far left pathway for this version of the figure, since this appendix is only about the right hand comparison
\begin{figure}
